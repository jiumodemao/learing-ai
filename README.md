...
同一 WiFi 下，家人手机浏览器直接使用：

1. 双击 `启动局域网版.bat`（窗口保持开启）
2. 窗口里会打印手机访问网址，形如 `http://192.168.x.x:8080`，把这个网址发给家人
3. 家人手机浏览器打开 → 注册自己的账号（邮箱验证已关闭，注册即用）→ 开始学习

注意事项：
- 需要你的电脑保持开机、窗口保持开启，家人才能访问
- 首次启动若弹出 Windows 防火墙提示，点"允许访问"
- 路由器重启后电脑 IP 可能变化，重启 .bat 看新网址即可
- 安全：服务器已封锁密钥目录（.secrets）、git 目录和私有知识库目录，家人只能访问应用页面
- 此方式无 HTTPS，不支持"添加到主屏幕"；如需公网访问与安装体验，后续可部署到 Cloudflare Pages
